...
import RewardPolicyManagement from './RewardPolicyManagement';
...
const RewardManagement: React.FC = () => {
    // Top-level tab state for Earning vs Usage
    const [managementMode, setManagementMode] = useState<'EARN' | 'USE'>('EARN');
    ...
    const handleAdd = () => {
        const newCode = 'RWD-' + Math.random().toString(36).substring(2, 8).toUpperCase();
        setSelectedReward({
    ...
    const columns = [
        ...
        },
        {
            ...
            accessor: (item: RewardItem) => (
                <span className="text-xs font-mono text-gray-500 bg-gray-50 px-2 py-1 rounded">{item.code}</span>
            ),
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex border-b border-gray-200">
                <button
                    onClick={() => setManagementMode('EARN')}
                    className={`px-6 py-3 font-bold text-sm transition-colors ${managementMode === 'EARN' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
                >
                    보상 지급 설정 (획득)
                </button>
                <button
                    onClick={() => setManagementMode('USE')}
                    className={`px-6 py-3 font-bold text-sm transition-colors ${managementMode === 'USE' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
                >
                    보상 사용 설정 (차감)
                </button>
            </div>
                                ...
                                <div className="p-6 space-y-6">
                                    ...
                                    <div className="flex items-start space-x-4 mb-4">
                                        ...
                                        <div className="flex-1 space-y-4">
                                            <div className="flex items-center space-x-2 mb-2 bg-gray-50 p-2 rounded justify-between">
                                                <span className="text-xs font-bold text-gray-500">API 연동 코드 (자동 생성)</span>
                                                <span className="text-sm font-mono font-bold text-gray-800">{selectedReward.code}</span>
                                            </div>
                                            <div>
